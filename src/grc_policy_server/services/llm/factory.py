...
def build_vllm_llm() -> VllmClient:
    return VllmClient(
        VllmSettings(
            ...
            write_timeout_sec=settings.vllm_write_timeout_sec,
...
_OPENAI_COMPATIBLE_PROVIDERS = frozenset({"vllm", "llamacpp", "openai_compatible", "openai"})


def build_llm() -> BaseLLM:
    """Build the runtime LLM client based on env config.

    Recognised LLM_PRIMARY_PROVIDER values:
      "ollama"            — Ollama only (no vLLM/llamacpp)
      "vllm"              — vLLM OpenAI-compatible server
      "llamacpp"          — llamacpp OpenAI-compatible server (same code path as vllm)
    ...
    # Default: vLLM primary with Ollama fallback.
    return FallbackLLM(primary=build_vllm_llm(), fallback=build_ollama_llm())
