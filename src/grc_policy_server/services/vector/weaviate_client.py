...
class WeaviateClient:
    ...
    def _ensure_schema(self) -> None:
        ...
        if not self.client.collections.exists(name):
            self.client.collections.create(
                ...
                vector_config=Configure.Vectors.text2vec_ollama(
                    ...
                    source_properties=["text", "section_path", "title"],
                ),
            )
            return
        ...
        existing = {
            prop.name
            for prop in self.client.collections.get(name).config.get(simple=True).properties
        }
    ...
    def _document_filter(
        ...
        *,
        ...
        if node_types:
            filters.append(
                Filter.any_of(
                    [Filter.by_property("node_type").equal(node_type) for node_type in node_types]
                )
            )
